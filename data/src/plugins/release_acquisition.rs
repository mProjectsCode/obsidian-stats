...
fn should_retry_main_js_download(entry: &PluginReleaseStateEntry) -> bool {
    entry.latest_release_fetch_status.as_deref() == Some("main_js_download_failed")
}
...
enum ReleaseFetchResult {
    NotModified,
    Updated(PluginReleaseStateEntry, Option<MainJsCacheOutcome>),
}
...
pub fn acquire_plugin_release_main_js(
    ...
    for plugin in plugins {
        ...
        if let Some(entry) = &previous_entry
            ...
            && entry
                ...
                .as_deref()
                .is_none_or(|status| status != "rate_limited")
    ...
    if !jobs.is_empty() {
        ...
        let results = thread_pool.install(|| {
            jobs.into_par_iter()
                .map(|job| process_release_job(job, &client, &rate_limit_mode))
                .collect::<Vec<_>>()
        });
...
fn process_release_job(
    ...
    let previous_etag = job
        .previous_entry
        .as_ref()
        .filter(|entry| !should_retry_main_js_download(entry))
        ...
        match parsed {
            Ok(latest) => {
                ...
                let (status, cache_outcome, successful_tag, successful_published_at) =
                    if let Some(asset) = main_js_asset {
                        if should_download_main_js_for_release(
                            ...
                            match save_main_js_to_cache(
                                ...
                                None => ("main_js_download_failed".to_string(), None, None, None),
                            }
                        } else {
                            (
...
fn save_main_js_to_cache(
    ...
    if let Ok(meta) = fs::metadata(&cache_path)
        && meta.len() == size
    {
        return Some(MainJsCacheOutcome::Reused);
    }
    ...
    if write_result.is_ok() {
        Some(MainJsCacheOutcome::Downloaded)
    } else {
        None
    }
}
